...
export class ConveniadoComponent implements OnInit {
  ...
  trackId(index: number, item: IConveniado): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IConveniado[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/conveniado'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
